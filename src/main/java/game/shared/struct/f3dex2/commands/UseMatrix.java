...
public class UseMatrix extends BaseF3DEX2
{
	...
	String[] optPush = new String[] { "NO_PUSH", "PUSH" };
	String[] optLoad = new String[] { "MULTIPLY", "LOAD" };
	String[] optProj = new String[] { "MODELVIEW", "PROJECTION" };
	...
	public String getString(BaseDataDecoder decoder)
	{
		if (decoder == null)
			...
			return String.format("%-16s (%s, %s, %s, %s)", getName(), decoder.getVariableName(addr),
				optPush[push ? 1 : 0], optLoad[load ? 1 : 0], optProj[proj ? 1 : 0]);
	}
}
