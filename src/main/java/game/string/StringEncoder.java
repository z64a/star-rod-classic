...
import static game.string.StringConstants.*;
...
import game.string.StringConstants.SpecialCharacter;
import game.string.StringConstants.StringEffect;
import game.string.StringConstants.StringFont;
import game.string.StringConstants.StringFunction;
import game.string.StringConstants.StringStyle;
import game.string.StringConstants.StringVoice;
import game.string.editor.StringTokenizer;
...
public class StringEncoder
{
	...
	private static final Matcher ClassicTagMatcher = ClassicTagPattern.matcher("");
	...
	public static List<PMString> parseStrings(StringResource res) throws IOException
	{
		if (res.usePatchFormat)
			return parsePatchFile(res);
		else
			return parseStringFile(res);
	}

	private static List<PMString> parseStringFile(StringResource res) throws IOException
	{
		List<PMString> messages = new ArrayList<>();

		List<Line> lines = IOUtils.readPlainInputFile(res.file);
		...
		for (PatchUnit unit : units) {
			...
			if (!declareLine.str.startsWith("#message:") && !declareLine.str.startsWith("#string:"))
				throw new InputFileException(declareLine, "Invalid string declaration: %n%s", declareLine.trimmedInput());
			...
			// #message | #string : section : index : (name)
			if (unit.declaration.numTokens() == 4) {
				try {
					section = unit.declaration.getHex(1);
				}
				catch (NumberFormatException e) {
					throw new InputFileException(declareLine, "String section could not be parsed: %n%s", declareLine.trimmedInput());
				}

				try {
					index = unit.declaration.getHex(2);
				}
				catch (NumberFormatException e) {
			...
			messages.add(new PMString(res, unit, section, index, name));
		}

		return messages;
	}

	private static List<PMString> parsePatchFile(StringResource res) throws IOException
	{
		List<PMString> messages = new ArrayList<>();
		...
		Pattern declarePattern = Pattern.compile("(?:#(?:string|message)|(?:@|#new:|#export:)(?:String|Message))\\s+\\$(\\S+)\\s*");
		Matcher declareMatcher = declarePattern.matcher("");
		...
		for (PatchUnit unit : units) {
			...
			declareMatcher.reset(unit.declaration.str);
			if (!declareMatcher.matches())
				throw new InputFileException(unit.declaration, "Can't parse string declaration: %n%s", unit.declaration.trimmedInput());
			...
			messages.add(new PMString(res, unit, name));
		}
	...
	private static void encode(StringEncoder builder, ArrayList<Sequence> sequences, boolean throwExceptions)
	{
		for (Sequence seq : sequences) {
			...
			catch (TagEncodingException | CharacterEncodingException e) {
			...
			switch (func) {
				...
				case FONT: // [font name]   or   [FONT:NAME]   with NAME = (normal|menu|title|subtitle)
				{
					if (!builder.allowFontChange)
						throw new TagEncodingException("Star Rod String Editor does not support setting font in Vars.");
				...
				case END_CHOICE:
					if (builder.doingSmartChoice) {
						...
						if (fields.length == 2) {
							...
							throw new TagEncodingException("Function %s has incorrect arg count: %d (expected 0 or 1).", fields[0], fields.length - 1);
		...
		if (classic) {
			...
			switch (func) {
				...
				case VOICE: { // [voice name]   or   [VOICE:NAME]   with NAME = (normal|bowser|spirit)
					...
					if (voice == null)
						throw new TagEncodingException("Function %s has invalid option: %s (expected 'normal', 'bowser', or 'spirit')", fields[0], fields[1]);
		...
		try {
			switch (pt) {
				case ANIMATION: { // [Animation spriteID=%X rasterIDs=%X,%X,%X,%X delays=%d,%d,%d,%d]
					...
					builder.tagBytes.addAll(seq.anim.getBytes());
				}
					break;
				...
				case CONST:
					if (fields.length != 2) // [Func:arg0]
						throw new TagEncodingException("Function %s has incorrect arg count: %d (expected 2).", fields[0], fields.length - 1);
					ByteBuffer constBytes = ProjectDatabase.getStringConstant(fields[1]);
					if (constBytes == null)
						throw new TagEncodingException("Unknown constant: %s", fields[1]);
					builder.addToTag(constBytes);
					break;
