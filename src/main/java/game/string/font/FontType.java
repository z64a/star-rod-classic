...
public enum FontType implements XmlSerializable
{
	...
	public static void patch(FontManager manager, RomPatcher rp)
	{
		for (FontType font : values()) {
			rp.seek("Font Data: " + font.name(), font.ptrData - PTR_TO_OFFSET);
			rp.writeByte(font.chars.texSize[0]);
			rp.writeByte(font.chars.texSize[1]);
			rp.writeByte(font.unk);
			rp.writeByte(font.lineHeight);
			rp.writeShort(font.chars.imgSize);
			rp.skip(2);
			int ptrCharData = rp.readInt();

			rp.seek("Font Variant Data: " + font.name(), ptrCharData - PTR_TO_OFFSET);

			for (int j = 0; j < font.numVariants; j++) {
				if (font.chars.patch_ptrRasters == 0)
					font.chars.patch_ptrRasters = rp.readInt();
				else
					rp.writeInt(font.chars.patch_ptrRasters);

				rp.skip(4); // ptrWidths
				rp.writeByte(font.fullspaceWidth[j]);
				rp.writeByte(font.baseHeightOffset[j]);
				rp.skip(2);
			}
		}
	}
