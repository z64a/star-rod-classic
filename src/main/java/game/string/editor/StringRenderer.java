...
import game.string.font.FontType;
...
public class StringRenderer
{
	...
	private int[] glItemPreviews = null;
	...
	private void loadTextures() throws IOException
	{
		...
		// background
		try {
			img = Tile.load(new File(DUMP_IMG_BG + "kmr_bg.png"), CI_8);
			bimg = ImageConverter.convertToBufferedImage(img);
			glBackgroundTexID = glLoadImage(bimg);
		}
		catch (IOException e) {
			throw new IOException("Could not background image!");
		}
	...
	private static void drawForegroundGuide(StringPrinter printer)
	{
		...
		v3 = LineRenderQueue.addVertex().setPosition(printer.windowBasePosX + printer.windowSizeX, printer.windowBasePosY + printer.windowSizeY, 0).getIndex();
	...
	public void render(MouseInput mouse, StringPrinter printer, float cameraX, float cameraY, float cameraZoom, float cameraYaw)
	{
		...
		while (printer.drawBuffer.position() < printer.printPos) {
			...
			if (charInt == StringPrinter.PAGE_END_MARK) {
				...
				int charWidth = MessageUtil.getCharWidth(state.currentFont, state.fontVariant, charInt, state.stringScaleX, state.charWidthOverride, 0);
	...
	private void handleFunction(StringPrinter printer, DrawState state, int relativePosX, int relativePosY)
	{
		...
		switch (func) {
			...
			case ANIM_DONE:
				if (printer.hasAnim()) {
					...
					if (npc != null && npc.rasters.getSize() > rasterID) {
						...
						raster.img.glBind(shader.texture);
						raster.defaultPal.pal.glBind(shader.palette);
						drawClippedQuad(shader, state, startX, startX + raster.img.width, startY, startY + raster.img.height);
					}
					else {
			...
			case ITEM_ICON:
				...
				if (glItemPreviews != null && glItemPreviews.length > itemID) {
					BasicTexturedShader shader = ShaderManager.use(BasicTexturedShader.class);
					shader.texture.bind(glItemPreviews[itemID]);
