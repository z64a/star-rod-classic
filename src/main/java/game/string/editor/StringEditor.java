...
import static game.string.StringConstants.CLOSE_TAG;
import static game.string.StringConstants.OPEN_TAG;
...
public class StringEditor extends BaseEditor
{
	...
	private SourceWatcher resourceWatcher = null;
	private ResourcePanel resourcePanel;
	private StringListPanel stringListPanel;
	...
	private PMString workingString;
	...
	// IO
	private ArrayList<FileEvent> pendingFileEvents = new ArrayList<>();
	public ArrayList<StringResource> resourcesToSave = new ArrayList<>();
	...
	public static void main(String[] args) throws IOException
	{
		...
		BaseEditor editor = new StringEditor();
		editor.launch();
		Environment.exit();
	}
	...
	{
		if (resourceWatcher != null && pendingFileEvents.isEmpty()) {
			if (saving) {
				// flush save events
				while (resourceWatcher.hasEvents())
					resourceWatcher.getNextEvent();
				saving = false;
			}

			if (resourceWatcher.hadOverflow()) {
				getMessageDialog("Too many file changes! All resources will be reloaded.", "File Changes Detected").show();
				...
				while (resourceWatcher.hasEvents()) {
					FileEvent evt = resourceWatcher.getNextEvent();
					if (evt.file.isDirectory() || FilenameUtils.isExtension(evt.file.getName(),
						new String[] { "str", "msg", "patch", "mpat", "bpat" }))
						pendingFileEvents.add(evt);
				}

				if (pendingFileEvents.size() > 0) {
					SwingUtilities.invokeLater(() -> {
						super.incrementDialogsOpen();
						FileChangesDialogPanel.showFramedDialog(getFrame(), pendingFileEvents);
						...
						invokeLater(() -> {
							...
							if (workingString != null && !workingString.hasValidSource())
								setString(null);
						});
					});
				}
	...
	protected JPanel createButtonPanel()
	{
		...
		for (SpecialCharacter sc : StringConstants.SpecialCharacter.values()) {
			...
			try {
				button.setIcon(getButtonIcon(imgFile, false));
				...
				Logger.logfWarning("Couldn't load image: %s", imgFile.getAbsolutePath());
			}
	...
	protected void createGui(JPanel toolPanel, Canvas glCanvas, JMenuBar menuBar, JLabel infoLabel, ActionListener openLogAction)
	{
		...
		stringListPanel = new StringListPanel(this);
		resourcePanel = new ResourcePanel(this, stringListPanel);

		JTabbedPane tabs = new JTabbedPane();
		createTab(tabs, "Resources", resourcePanel);
	...
	private JPanel makeConstantsTab()
	{
		JPanel settingsPanel = new JPanel(new MigLayout("fill, ins 0, wrap", "[10%!][][10%!]"));

		//TODO
		settingsPanel.add(SwingUtils.getLabel("SOON", 14));
	...
	public void setString(PMString selectedValue)
	{
		...
		SwingUtilities.invokeLater(() -> {
			stringListPanel.setString(selectedValue);
		...
		if (workingString != null)
			Logger.log("Editing " + (workingString.hasID() ? workingString.getIDName() : workingString.name));
	}
	...
	private void addEditorMenu(JMenuBar menuBar)
	{
		...
		awtKeyStroke = KeyStroke.getKeyStroke(KeyEvent.VK_Z, KeyEvent.CTRL_DOWN_MASK);
		item.setAccelerator(awtKeyStroke);
		item.getInputMap(JMenuItem.WHEN_IN_FOCUSED_WINDOW).put(awtKeyStroke, "none");
		item.addActionListener((e) -> {
			undoEDT();
		});
		menu.add(item);
		...
		awtKeyStroke = KeyStroke.getKeyStroke(KeyEvent.VK_Y, KeyEvent.CTRL_DOWN_MASK);
		item.setAccelerator(awtKeyStroke);
		item.getInputMap(JMenuItem.WHEN_IN_FOCUSED_WINDOW).put(awtKeyStroke, "none");
		item.addActionListener((e) -> {
			redoEDT();
