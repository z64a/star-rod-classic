...
public class WorldMapEditor extends BaseEditor implements MouseManagerListener
{
	...
	protected void createGui(JPanel toolPanel, Canvas glCanvas, JMenuBar menubar, JLabel infoLabel, ActionListener openLogAction)
	{
		...
		locationsBox.addActionListener((e) -> {
			if (selectedLocation != null)
				selectedLocation.locationName = (String) locationsBox.getSelectedItem();
		});
	...
	private void addViewMenu(JMenuBar menuBar)
	{
		...
		cbLines.getInputMap(JMenuItem.WHEN_IN_FOCUSED_WINDOW).put(dummyKeyStroke, "none");
		menu.add(cbLines);
		...
		cbIcons.getInputMap(JMenuItem.WHEN_IN_FOCUSED_WINDOW).put(dummyKeyStroke, "none");
		menu.add(cbIcons);
				...
				if (loc == parentingMarker) {
					if (framePick != null) {
						LineRenderQueue.addLine(
							...
							LineRenderQueue.addVertex().setPosition(framePick.worldPos.x, framePick.worldPos.y, 0).getIndex());
					}
				}
				else {
					if (loc.parent != null) {
						LineRenderQueue.addLine(
							LineRenderQueue.addVertex().setPosition(loc.parent.getX(), loc.parent.getY(), 0).getIndex(),
							LineRenderQueue.addVertex().setPosition(loc.getX(), loc.getY(), 0).getIndex());
					}
				}
			}
	...
	{
		if (mouseMarker instanceof WorldLocation) {
	...
	public void stopHoldingRMB()
	{
		if (parentingMarker != null) {
			if (mouseMarker != null) {
				if (mouseMarker instanceof WorldLocation)
					((WorldLocation) parentingMarker).parent = ((WorldLocation) mouseMarker);
				else if (mouseMarker instanceof WorldPathElement)
					((WorldLocation) parentingMarker).parent = ((WorldPathElement) mouseMarker).owner;
			}
	...
	private void setSelectedLocation(WorldLocation loc)
	{
		...
		if (selectedLocation != null) {
			locationsBox.setSelectedItem(selectedLocation.locationName);
		}
	}
}
