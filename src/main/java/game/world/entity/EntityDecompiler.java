...
import game.shared.struct.f3dex2.commands.SetOtherModeL.OtherModeOption;
...
import game.world.entity.EntityModel.EntityPart;
import game.world.entity.EntityModel.EntityTexture;
import game.world.entity.EntityModel.EntityTriangle;
import game.world.entity.EntityModel.EntityVertex;
import util.Logger;

public class EntityDecompiler
{
	...
	public static void decompileAll()
	{
		for (EntityType entity : EntityType.values()) {
			...
			try {
				new EntityDecompiler(entity.name);
			}
			catch (IOException e) {
	...
	public EntityDecompiler(String name) throws IOException
	{
		entityName = name;
		...
		new EntityDecoder(entityName, fileBuffer, roots);
	...
	/*
	public EntityDecompiler() throws IOException
	{
		// dumps shadows
		shadowMode = true;
	...
	*/
		...
		for (BaseF3DEX2 cmd : commands) {
			switch (cmd.type) {
				case G_DL:
					...
					if (PRINT_DISPLAY_LISTS)
						Logger.log(indent + cmd.getString(null));
	...
	private static EntityTriangle buildTriangle(EntityVertex[] vertexTable, int i, int j, int k)
	{
		...
		EntityTriangle t = new EntityTriangle(v1, v2, v3);
		return t;
	}
