...
public class AudioTest3
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		engine.addVoice(new Voice(sound));
	...
	private static class Voice
	{
		...
		public void renderInto(float[] bufferL, float[] bufferR, int numSamples, int outputRate)
		{
			...
			float resampleRatio = pitch * ((float) sound.sampleRate / outputRate);
			...
			for (int i = 0; i < numSamples; i++) {
				...
				float s0 = (float) sound.samples.get(i0) / Short.MAX_VALUE;
				float s1 = (float) sound.samples.get(i1) / Short.MAX_VALUE;
	...
	private static class Sound
	{
		...
		public Sound(File wavFile) throws Exception
		{
			try (AudioInputStream audioInputStream = AudioSystem.getAudioInputStream(wavFile)) {
				...
				samples = new ArrayList<>(audioBytes.length / Short.BYTES);
				...
				Logger.logf("Loaded WAV: %d samples @ %s Hz%n", samples.size(), sampleRate);

				if (samples.isEmpty())
					throw new IllegalArgumentException("WAV file contained no samples.");
			}
		}
	}
}
