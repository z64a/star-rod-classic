...
public class ImageEditor extends BaseEditor implements MouseManagerListener, ColorUpdateListener
{
	...
	protected void createGui(JPanel toolPanel, Canvas glCanvas, JMenuBar menubar, JLabel infoLabel, ActionListener openLogAction)
	{
		...
		colorSwatchesScrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		colorSwatchesScrollPane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
	...
	{
		int choice = getOptionDialog("Export Image", "File already exists: " + file.getName())
	...
	private void newImageEDT()
	{
		...
		int choice = getOptionDialog("New Image Options", createOptions)
			.choose();
