...
public class SearchBushEvent extends BaseStruct
{
	...
	public void scan(BaseDataDecoder decoder, Pointer ptr, ByteBuffer fileBuffer)
	{
		if (!ptr.nameWasOverriden() && decoder instanceof MapDecoder mapDecoder) {
			mapDecoder.bushCount++;
			ptr.foliageName = "Bush" + mapDecoder.bushCount;
		...
		enqueueAndName(decoder, ptr, fileBuffer.getInt(), TreeModelListT, "Bush");
