...
public class VolumeComponent extends BaseMarkerComponent
{
	...
	public void toBinary(ObjectOutput out) throws IOException
	{
		...
		out.writeFloat(height.get());
		out.writeObject(minPos.point);
		out.writeObject(maxPos.point);
	}
	...
	public void fromBinary(ObjectInput in) throws IOException, ClassNotFoundException
	{
		...
		height.set(in.readFloat());

		point = (MutablePoint) in.readObject();
		minPos = new SelectablePoint(point, 2.0f);

		point = (MutablePoint) in.readObject();
		maxPos = new SelectablePoint(point, 2.0f);
	}
