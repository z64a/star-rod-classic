...
public class CamTargetComponent extends BaseMarkerComponent
{
	...
	{
		out.writeBoolean(useZone.get());
		out.writeBoolean(generatePan.get());
		out.writeFloat(moveSpeed.get());

		if (useZone.get()) {
			out.writeBoolean(overrideDist.get());
			out.writeFloat(boomLength.get());

			out.writeBoolean(overrideAngles.get());
			out.writeFloat(boomPitch.get());
			out.writeFloat(viewPitch.get());
		}
		else
			controlData.toBinary(out);
	}

	@Override
	public void fromBinary(ObjectInput in) throws IOException, ClassNotFoundException
	{
		useZone.set(in.readBoolean());
		generatePan.set(in.readBoolean());
		moveSpeed.set(in.readFloat());

		if (useZone.get()) {
			overrideDist.set(in.readBoolean());
			boomLength.set(in.readFloat());

			overrideAngles.set(in.readBoolean());
			boomPitch.set(in.readFloat());
			viewPitch.set(in.readFloat());
		}
		else
			controlData = CameraZoneData.fromBinary(parentMarker, in);
	}
