...
public class GridComponent extends BaseMarkerComponent
{
	...
	public void toBinary(ObjectOutput out) throws IOException
	{
		...
		out.writeInt(gridOccupants.size());
		for (GridOccupant occ : gridOccupants) {
			if (occ.posX < gridSizeX.get() && occ.posZ < gridSizeZ.get()) {
				out.writeInt(occ.posX);
				out.writeInt(occ.posZ);
				out.writeInt(occ.type.get().id);
			}
		}
		out.writeBoolean(gridUseGravity.get());
	}
	...
	public void fromBinary(ObjectInput in) throws IOException, ClassNotFoundException
	{
		...
		int numOccupants = in.readInt();
		for (int i = 0; i < numOccupants; i++) {
			int posX = in.readInt();
			int posZ = in.readInt();
			int id = in.readInt();
			gridOccupants.add(new GridOccupant(gridOccupants, posX, posZ, id));
		}
		gridUseGravity.set(in.readBoolean());
	}
	...
	{
		Element gridElem;
		if (xmr.hasAttribute(markerElem, ATTR_MARKER_GRID)) //XXX temporary support for old save format
			gridElem = markerElem;
		else
