...
public class MapEditor extends GLEditor implements MouseManagerListener, KeyboardInputListener, KeyEventDispatcher
{
	...
	private void cleanup(boolean crashed)
	{
		...
		if (crashed) {
			if (saveCrashFile())
				Logger.log("Saved crash file to " + crashFile.getAbsolutePath());
			else
				Logger.log("Failed to save crash file!");
	...
	private void loadRecentMaps()
	{
		...
		for (Options opt : opts) {
			...
			if (mapName != null && !mapName.isBlank() && new File(mapName).exists()) {
	...
	{
		String current = map.getFilePath();
	...
	private void saveBackup()
	{
		...
		try {
			...
			Logger.logError("Backup failed for " + map.name);
	...
	private Map checkForCrashData()
	{
		...
		try {
			Map crashMap = Map.loadBackup(crashFile);
			if (crashMap == null)
				return null;

			int choice = SwingUtils.getConfirmDialog()
				.setTitle("Crash Recovery")
				.setMessage("Found crash data for " + crashMap.name, "Would you like to load it?")
	...
	private Map checkForBackup(Map newMap)
	{
		try {
			...
			Map backupMap = Map.loadBackup(backupFile);
			if (backupMap == null) {
			...
			int choice = SwingUtils.getConfirmDialog()
				...
				.setMessage("Found backup data for " + newMap.name, "Would you like to load it instead?")
				.setMessageType(JOptionPane.PLAIN_MESSAGE)
				.setOptionsType(JOptionPane.YES_NO_OPTION)
				.choose();
	...
	private Map showOpeningPrompt()
	{
		...
		if (editorConfig != null) {
			...
			if (lastFilename != null && !lastFilename.isBlank()) {
				...
				if (lastMap.exists()) {
					String lastName = lastMap.getName();
					options = new String[] { "Browse Maps", "Reopen " + lastName };
				}
			}
		}
		...
		while (true) {
			...
			if (choice == 0) {
				if (gui.openMapChooser.prompt() == ChooseDialogResult.APPROVE) {
					File selectedMapFile = gui.openMapChooser.getSelectedFile();
					if (selectedMapFile != null)
						selectedMap = Map.loadMap(selectedMapFile);
					break;
				}
			}
		...
		switch (cmd) {
			...
			case COMPILE_SHAPE:
				try {
					...
					Logger.log("Building " + shapeMap.name + "_shape...");
					new GeometryCompiler(shapeMap);
					Logger.log("Successfully compiled " + shapeMap.name + "_shape");
				}
			...
			case COMPILE_COLLISION:
				try {
					...
					Logger.log("Building " + hitMap.name + "_hit...");
					new CollisionCompiler(hitMap);
					Logger.log("Successfully compiled " + hitMap.name + "_hit");
				}
				catch (BuildException be) {
					SwingUtils.getErrorDialog()
						.setParent(gui)
	...
	private void loadMapResources(boolean reloadTextures)
	{
		...
		while (!loadedTextures) {
			...
			if (choice == JOptionPane.YES_OPTION) {
				if (gui.texFileChooser.prompt() == ChooseDialogResult.APPROVE) {
					File f = gui.texFileChooser.getSelectedFile();
					if (f != null) {
		...
		try {
			if (map.hasBackground) {
				...
				File png = AssetManager.getBackground(map.bgName);

				if (png != null) {
					map.bgImage = ImageIO.read(png);
					map.glBackgroundTexID = TextureManager.bindBufferedImage(map.bgImage);
				}
				else {
					Logger.log("Unable to locate background image!", Priority.WARNING);
				}
			}
			else
	...
	public void action_SaveMapAs(File f)
	{
		if (!f.exists()) {
			...
			if (!name.endsWith(Map.EXTENSION))
				f = new File(f.getAbsolutePath() + Map.EXTENSION);
		}

		try {
			map.saveMapAs(f, getAuthor(), new MapEditorMetadata(this));
		}
		catch (Exception e) {
			displayStackTrace(e);
			return;
	...
	private void checkShapeOverride()
	{
		...
		if (map.scripts.overrideShape.get()) {
			...
			File[] matches = IOUtils.getFileWithin(MOD_MAP_SAVE, overrideName + Map.EXTENSION, true);
			File overrideSaveFile = matches.length > 0 ? matches[0] : null;

			matches = IOUtils.getFileWithin(MOD_MAP_SRC, overrideName + Map.EXTENSION, true);
			File overrideSrcFile = matches.length > 0 ? matches[0] : null;

			File f = (overrideSaveFile != null) ? overrideSaveFile : (overrideSrcFile != null) ? overrideSrcFile : null;
			if (f == null) {
				Logger.logError("Couldn't find map: " + overrideName + Map.EXTENSION);
				shapeOverride = null;
				return;
			}

			shapeOverride = Map.loadMap(f);
			TextureManager.assignModelTextures(shapeOverride);
			Logger.log("Loaded override geometry: " + shapeOverride.name);
		}
		else
			shapeOverride = null;
	}
	...
	private void checkHitOverride()
	{
		if (map.scripts.overrideHit.get()) {
			...
			if (shapeOverride != null && overrideName.equals(shapeOverride.name))
				hitOverride = shapeOverride;
			else
				hitOverride = Map.loadMap(f);

			Logger.log("Loaded override collision: " + hitOverride.name);
		}
		else
			hitOverride = null;
	}
	...
	{
		gui.setTitle(Environment.decorateTitle("Map Editor") + " - " + map.source.getName());
	}
	...
	private void copyObjects()
	{
		...
		try {
			int numCopied = map.exportPrefab(clipboardFile, getAuthor(), true);
			if (numCopied > 0)
	...
	public void loadPreferences()
	{
		...
		author = editorConfig.getString(Options.Author);
	...
	public void displayStackTrace(Throwable t, String errorMessage)
	{
		SwingUtilities.invokeLater(() -> {
			...
			StarRodClassic.displayStackTrace(t);
