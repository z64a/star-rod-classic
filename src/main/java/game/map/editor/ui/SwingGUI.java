...
public final class SwingGUI extends StarRodFrame implements ActionListener, Logger.Listener, IShutdownListener
{
	...
	public OpenFileChooser openMapChooser;
	public SaveFileChooser saveMapChooser;
	...
	public SwingGUI(MapEditor editor, EditorCanvas glCanvas, File logFile)
	{
		...
		snapModeLabel = new JLabel("", JLabel.LEFT);
		gridSizeLabel = new JLabel("", JLabel.RIGHT);
		extraInfoLabel = new JLabel("");
		fpsLabel = new JLabel("", JLabel.RIGHT);
	...
	private JPanel createTextureTab()
	{
		...
		textureScrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
	...
	private void prompt_OpenMap()
	{
		if (!editor.map.modified || promptForSave()) {
			openDialogCount.increment();
			if (openMapChooser.prompt() == ChooseDialogResult.APPROVE) {
				final File f = openMapChooser.getSelectedFile();
				if (f.exists())
					editor.doNextFrame(() -> {
						editor.action_OpenMap(f);
					});
			}
			openDialogCount.decrement();
		}
	...
	{
		openDialogCount.increment();
		if (saveMapChooser.prompt() == ChooseDialogResult.APPROVE) {
			final File f = saveMapChooser.getSelectedFile();
			editor.doNextFrame(() -> {
				editor.action_SaveMapAs(f);
			});
		}
		openDialogCount.decrement();
	}
	...
	{
		openDialogCount.increment();
		if (texFileChooser.prompt() == ChooseDialogResult.APPROVE) {
	...
	{
		openDialogCount.increment();
		if (bgFileChooser.prompt() == ChooseDialogResult.APPROVE) {
			File f = bgFileChooser.getSelectedFile();
			if (f != null) {
				editor.doNextFrame(() -> {
					try {
						String bgName = FilenameUtils.getBaseName(f.getName());
	...
	private void prompt_ExportObjects()
	{
		...
		if (exportFileChooser.prompt() == ChooseDialogResult.APPROVE) {
			...
			editor.map.exportToFile(f, editor.getAuthor());
		}
		openDialogCount.decrement();
	}
