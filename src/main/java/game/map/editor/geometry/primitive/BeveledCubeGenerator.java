...
public class BeveledCubeGenerator extends ShapeGenerator
{
	...
	private static TriangleBatch generate(int size, int bevelLength, boolean sharpMiter, int centerX, int centerY, int centerZ)
	{
		...
		float x = (Lb/2.0f);
		uForCol[0] = Math.round(x/effectiveSize * UV_SCALE); x += Ls;
		uForCol[1] = Math.round(x/effectiveSize * UV_SCALE); x += Lb;
		uForCol[2] = Math.round(x/effectiveSize * UV_SCALE); x += Ls;
		uForCol[3] = Math.round(x/effectiveSize * UV_SCALE); x += Lb;
		uForCol[4] = Math.round(x/effectiveSize * UV_SCALE); x += Ls;
		uForCol[5] = Math.round(x/effectiveSize * UV_SCALE); x += Lb;
		uForCol[6] = Math.round(x/effectiveSize * UV_SCALE); x += Ls;
		uForCol[7] = Math.round(x/effectiveSize * UV_SCALE); x += Lb;
