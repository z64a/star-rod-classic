...
public class ShadowRenderer
{
	...
	private static final void convertImage(BufferedImage image)
	{
		// simple conversion for I-4 image --> black, alpha-modulated image suitable to be rendered as a shadow
		for (int i = 0; i < image.getHeight(); i++)
			for (int j = 0; j < image.getWidth(); j++) {
				int pixel = image.getRGB(i, j);
				// A is always FF, all channels are the same, so just grab blue
				int B = (pixel & 0xFF);
				// use black for the color
				image.setRGB(i, j, B << 24);
			}
	}
	...
	public static void init()
	{
		...
		try {
			BufferedImage image = ImageIO.read(new File(Directories.MOD_IMG_ASSETS + "/shadow/circle.png"));
			convertImage(image);
		...
		try {
			BufferedImage image = ImageIO.read(new File(Directories.MOD_IMG_ASSETS + "/shadow/square.png"));
			convertImage(image);
	...
	private static void draw(Vector3f pos, Vector3f norm, float height, boolean battle, boolean circularShadow, float baseScale)
	{
		...
		if (texID == -1) {
			...
			shader.setQuadTexCoords(0.0f, 0.0f, 2.0f, 2.0f);
