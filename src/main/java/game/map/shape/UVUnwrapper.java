...
public class UVUnwrapper
{
	...
	public UVUnwrapper(MapEditor editor, Iterable<Triangle> triangles)
	{
		...
		for (Triangle t : triangles) {
			...
			Vector3f triCenter = t.getCenter();
			center[0] += triCenter.x;
			center[1] += triCenter.y;
			center[2] += triCenter.z;
		}

		for (UnwrapVert v : verts.keySet()) {
			double dx = v.meshVertex.getCurrentX() - center[0];
			double dy = v.meshVertex.getCurrentY() - center[1];
			double dz = v.meshVertex.getCurrentZ() - center[2];
