...
public class TransformMatrix implements Serializable, XmlSerializable
{
	...
	public void readRDP(ByteBuffer bb)
	{
		...
		{
			for(int j = 0; j < 4; j++)
				System.out.printf("%04X.%04X ", whole[i][j], frac[i][j]);
			System.out.println();
		}
		...
		{
			for(int j = 0; j < 4; j++)
				System.out.printf("%9.2f ", mat[i][j]);
			System.out.println();
		}
		 */
	}
	...
	public boolean equals(Object obj)
	{
		...
		if (obj == null)
			return false;
		if (!(obj instanceof TransformMatrix other))
			return false;
	...
	{
		Vector3f forward = Vector3f.sub(eye, obj);
