...
public class Model extends MapObject
{
	...
	public EditableField<Boolean> hasRenderMode = EditableFieldFactory.create(false)
		.setCallback(notifyCallback).setName(new StandardBoolName("Render Mode")).build();
	...
	public void fromXML(XmlReader xmr, Element mdlElem)
	{
		...
		else if (xmr.hasAttribute(mdlElem, TEMP_ATTR_SCROLL))
			pannerID.set(xmr.readInt(mdlElem, TEMP_ATTR_SCROLL)); //XXX
	}
	...
	public void toXML(XmlWriter xmw)
	{
		...
		for (int i = 0; i < properties.length; i++) {
			...
			xmw.addHexArray(propertyTag, ATTR_PROPERTY_V, properties[i][0], properties[i][1], properties[i][2]);
