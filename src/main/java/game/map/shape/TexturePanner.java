...
/*
	...
	3 = aux dU
	4 = aux dV
	5 = main U rate (scroll every N frames, 0 for none)
	6 = main dV rate (scroll every N frames, 0 for none)
	7 = aux U rate (scroll every N frames, 0 for none)
	8 = aux V rate (scroll every N frames, 0 for none)
	9 = initial main U --  always zero, only set when 5-8 all = 1
	A = always zero
	B = always zero
	C = always zero
*/
...
public class TexturePanner
{
	...
	public void load(Integer[] args)
	{
		params.rate[MAIN_U] = args[1];
		params.rate[MAIN_V] = args[2];
		params.rate[AUX_U] = args[3];
		params.rate[AUX_V] = args[4];

		params.freq[MAIN_U] = args[5];
		params.freq[MAIN_V] = args[6];
		params.freq[AUX_U] = args[7];
		params.freq[AUX_V] = args[8];

		params.init[MAIN_U] = args[9];
		params.init[MAIN_V] = args[10];
		params.init[AUX_U] = args[11];
	...
	public static void load(DefaultListModel<TexturePanner> texPanUnits, XmlReader xmr, Element pannerElem)
	{
		...
		// loading legacy params if present
		loadArray(xmr, pannerElem, ATTR_PAN_MAIN_INIT, panner.params.init, false);
		loadArray(xmr, pannerElem, ATTR_PAN_AUX_INIT, panner.params.init, true);
		loadArray(xmr, pannerElem, ATTR_PAN_MAIN_STEP, panner.params.rate, false);
		loadArray(xmr, pannerElem, ATTR_PAN_AUX_STEP, panner.params.rate, true);

		if (xmr.hasAttribute(pannerElem, ATTR_PAN_MAIN_TICK)) {
			int[] vals = xmr.readIntArray(pannerElem, ATTR_PAN_MAIN_TICK, 2);
			panner.params.freq[MAIN_U] = vals[0];
			panner.params.freq[MAIN_V] = vals[1];
		}

		if (xmr.hasAttribute(pannerElem, ATTR_PAN_AUX_TICK)) {
			int[] vals = xmr.readIntArray(pannerElem, ATTR_PAN_AUX_TICK, 2);
			panner.params.freq[AUX_U] = vals[0];
			panner.params.freq[AUX_V] = vals[1];
		}

		// load newer param format
	...
	private static void loadArray(XmlReader xmr, Element pannerElem, XmlKey key, int[] array, boolean isAux)
	{
		boolean useTexels = false;
		int auxOffset = isAux ? 2 : 0;

		int[] vals; // have to copy values since arrays are final
		if (xmr.hasAttribute(pannerElem, key)) {
			vals = xmr.readIntArray(pannerElem, key, 2);
			for (int i = 0; i < vals.length; i++) {
				if (useTexels) {
					array[MAIN_U + auxOffset + i] = vals[i] * TEXEL_RATIO;
					array[MAIN_S + auxOffset + i] = vals[i];
				}
				else {
					array[MAIN_U + auxOffset + i] = vals[i];
					array[MAIN_S + auxOffset + i] = vals[i] / TEXEL_RATIO;
				}
			}
		}
	}
