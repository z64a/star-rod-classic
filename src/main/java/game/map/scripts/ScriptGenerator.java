...
import static app.Directories.MOD_MAP_GEN;
...
public class ScriptGenerator
{
	...
	public ScriptGenerator(Map map) throws IOException, InvalidInputException
	{
		...
		String patchName = map.name + ".mpat";

		File gen = new File(MOD_MAP_GEN + patchName);
	...
	private void addInitScript(List<String> lines) throws InvalidInputException
	{
		...
		lines.add("#new:Function_Init $Function_Init");
		lines.add("{");
		lines.add("\tPUSH    RA");
		...
		if (map.scripts.overrideShape.get()) {
			...
			lines.add("\tLIA     A0, 800D9230");
			lines.add("\tLIA     A1, \"" + assetName + "\"");
			lines.add("\tJAL     800654F0 % sprintf");
			lines.add("\tRESERVED");
		}
	...
	private void addEntryList(List<String> lines)
	{
		...
		for (Marker m : index.getEntryList())
			lines.add(String.format("\t%cVec4f:%s", SyntaxConstants.EXPRESSION_PREFIX, m.getName()));
		lines.add("}");
		lines.add("");
	}
	...
	{
		lines.add("#new:Script_Main " + nameMain);
		lines.add("{");
			...
			for (Entrance e : entrances) {
				...
				switch (e.type.get()) {
					...
					case BlueWarpPipe:
						...
						String flagName = entity.entityComponent.flagName.get();
					...
					if (lockName == null || lockName.isEmpty()) {
						...
						switch (m.entityComponent.type.get()) {
							...
							case PadlockBlueFace:
								...
								int mapVar = m.entityComponent.autoMapVar.get() ? getNextMapVarIndex() : m.entityComponent.mapVarIndex.get();
								...
								lines.add(String.format("\t\tBindLock  $Script_CheckLock_%s .Trigger:WallPressA 00004000 $ItemList_%s 00000000 00000001",
									m.getName(), m.getName()));
								lines.add("\tElse");
								lines.add("\t" + bindDoorLine);
								lines.add("\tEndIf");
	...
	{
		List<String> lines = Resource.getText(ResourceType.ScriptGenTemplate, name);
