...
public final class Exit extends Generator
{
	...
	public Exit(ExitType type)
	{
		...
		doorSound = EditableFieldFactory.create(ProjectDatabase.DoorSoundsType.getName(0))
			.setCallback((o) -> {
				ExitInfoPanel.instance().updateFields(this);
			}).setName("Set Door Sounds").build();

		doorSwing = EditableFieldFactory.create(ProjectDatabase.DoorSwingsType.getName(1))
			.setCallback((o) -> {
				ExitInfoPanel.instance().updateFields(this);
	...
	public Exit(ExitType type, ArrayList<ScriptLine> script)
	{
		this(type);
		...
		switch (type) {
			...
			case SingleDoor:
				...
				colliderID = script.get(4).args[1];
				door1ID = script.get(5).args[1];
				doorSound.set(ProjectDatabase.DoorSoundsType.getName(script.get(2).args[1]));
				doorSwing.set(ProjectDatabase.DoorSwingsType.getName(script.get(6).args[1]));
		...
		switch (type.get()) {
			case SingleDoor:
				...
				if (xmr.hasAttribute(elem, ATTR_DOOR_DIR)) {
					...
					doorSwing.set(ProjectDatabase.DoorSwingsType.getName(doorSwingDir));
				}

				if (xmr.hasAttribute(elem, ATTR_DOOR_SFX)) {
					int doorSoundID = xmr.readHex(elem, ATTR_DOOR_SFX);
					doorSound.set(ProjectDatabase.DoorSoundsType.getName(doorSoundID));
				}
				break;
	...
	public void toXML(XmlWriter xmw)
	{
		...
		switch (type.get()) {
			case SingleDoor:
				...
				xmw.addInt(exitTag, ATTR_DOOR_DIR, ProjectDatabase.DoorSwingsType.getID(doorSwing.get()));
				xmw.addHex(exitTag, ATTR_DOOR_SFX, ProjectDatabase.DoorSoundsType.getID(doorSound.get()));
				break;
