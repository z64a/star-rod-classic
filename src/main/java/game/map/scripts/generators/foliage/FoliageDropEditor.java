...
public class FoliageDropEditor extends JPanel
{
	...
	static {
		...
		for (int i = 0; i < AREA_FLAG_NAMES.length; i++)
			AREA_FLAG_NAMES[i] = ScriptVariable.getString(ScriptVariable.AreaFlag, i);
	}
	...
	public FoliageDropEditor(Map map, FoliageDrop original)
	{
		...
		ConstEnum spawnModeEnum = ProjectDatabase.getFromNamespace("ItemSpawnMode");
		...
		itemNameBox = new JComboBox<>(ProjectDatabase.getItemNames());
		...
		List<String> modFlagNames = ProjectDatabase.getModFlagList();
