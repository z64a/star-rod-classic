...
public class ScriptData extends UpdateProvider implements XmlSerializable
{
	...
	public ScriptData()
	{
		...
		songName = EditableFieldFactory.create("PleasantPath")
		...
		ambientSFX = EditableFieldFactory.create("Wind")
			.setCallback(notifyGeneral).setName("Set Ambient Sounds").build();

		locationName = EditableFieldFactory.create("GoombaRoad")
	...
	public void fromXML(XmlReader xmr, Element scriptElem)
	{
		...
		if (optionsElem != null) {
			...
			if (xmr.hasAttribute(optionsElem, ATTR_SONG)) {
				int songID = xmr.readHex(optionsElem, ATTR_SONG);
				songName.set(ProjectDatabase.getFromNamespace("Song").getName(songID));
			}
			...
			if (xmr.hasAttribute(optionsElem, ATTR_SOUNDS)) {
				int sfxID = xmr.readHex(optionsElem, ATTR_SOUNDS);
				ambientSFX.set(ProjectDatabase.getFromNamespace("AmbientSounds").getName(sfxID));
			}

			if (xmr.hasAttribute(optionsElem, ATTR_LOCATION)) {
				int locationID = xmr.readHex(optionsElem, ATTR_LOCATION);
				locationName.set(ProjectDatabase.LocationType.getName(locationID));
			}
	...
	public void toXML(XmlWriter xmw)
	{
		...
		Integer locationID = ProjectDatabase.LocationType.getID(locationName.get());
		if (locationID == null)
			throw new StarRodException("Undefined location for map: " + locationName.get());
		xmw.addHex(optionsTag, ATTR_LOCATION, locationID);

		xmw.addBoolean(optionsTag, ATTR_HAS_MUSIC, hasMusic.get());
		if (hasMusic.get() && songName.get() != null && !songName.get().isEmpty()) {
			Integer songID = ProjectDatabase.getFromNamespace("Song").getID(songName.get());
			if (songID == null)
				throw new StarRodException("Undefined background music track for map: " + songName.get());
			xmw.addHex(optionsTag, ATTR_SONG, songID);
		}

		xmw.addBoolean(optionsTag, ATTR_HAS_SOUNDS, hasAmbientSFX.get());

		Integer sfxID = ProjectDatabase.getFromNamespace("AmbientSounds").getID(ambientSFX.get());
		if (sfxID == null)
			throw new StarRodException("Undefined ambient sounds for map: " + ambientSFX.get());
		xmw.addHex(optionsTag, ATTR_SOUNDS, sfxID);
