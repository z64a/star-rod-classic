...
public class CameraZoneData extends UpdateProvider implements Externalizable, XmlSerializable
{
	...
	public void toBinary(ObjectOutput out) throws IOException
	{
		...
		out.writeFloat(boomLength.get());
		out.writeFloat(boomPitch.get());
		out.writeFloat(viewPitch.get());

		out.writeInt(posA.getX());
		out.writeInt(posA.getY());
		out.writeInt(posA.getZ());

		out.writeInt(posB.getX());
		out.writeInt(posB.getY());
		out.writeInt(posB.getZ());

		out.writeInt(posC.getX());
		out.writeInt(posC.getY());
		out.writeInt(posC.getZ());
	}

	public static CameraZoneData fromBinary(Marker parentMarker, ObjectInput in) throws IOException, ClassNotFoundException
	{
		CameraZoneData data = new CameraZoneData(parentMarker);

		data.setType(ControlType.values()[in.readInt()]);
		data.setFlag(in.readBoolean());

		data.boomLength.set(in.readFloat());
		data.boomPitch.set(in.readFloat());
		data.viewPitch.set(in.readFloat());

		data.posA = new SelectablePoint(new MutablePoint(in.readInt(), in.readInt(), in.readInt()), 2.0f);
		data.posB = new SelectablePoint(new MutablePoint(in.readInt(), in.readInt(), in.readInt()), 2.0f);
		data.posC = new SelectablePoint(new MutablePoint(in.readInt(), in.readInt(), in.readInt()), 2.0f);

		data.points.add(data.posA);
		data.points.add(data.posB);
		data.points.add(data.posC);
	...
	{
		throw new IllegalStateException("Use toBinary instead!");
	}

	@Override
	public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException
	{
		throw new IllegalStateException("Use fromBinary instead!");
	}
	...
	private void drawHelpers2(Renderer renderer, boolean previewMode, float h)
	{
		...
		if (vBA.length() > 0) {
			vBA.normalize();
			endPoint = Vector3f.cross(new Vector3f(0.0f, 64.0f, 0.0f), vBA);
	...
	private void drawHelpers6(Renderer renderer, boolean previewMode, float h)
	{
		if (previewMode && !flag) {
			...
			if (vBA.length() > 0) {
				vBA.normalize();
				Vector3f unit = Vector3f.cross(new Vector3f(0.0f, 1.0f, 0.0f), vBA);
