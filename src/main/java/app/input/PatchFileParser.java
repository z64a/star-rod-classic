...
public class PatchFileParser
{
	...
	public static final void main(String[] args) throws IOException
	{
		...
		File patchFile = new File(Directories.MOD_PATCH + "/TEST.patch");
		List<Line> lines = IOUtils.readPlainInputFile(patchFile);
		List<PatchUnit> units;

		CaseInsensitiveMap<String> rules = new CaseInsensitiveMap<>();
		rules.put("npcID", "1");

		try {
			units = PatchFileParser.parse(lines, rules);
		}
		...
		for (PatchUnit unit : units) {
			System.out.println("==========================");
			System.out.println(unit.declaration.str);
			System.out.println("--------------------------");
	...
	{
		return parse(lines, new CaseInsensitiveMap<>());
	}
	...
	{
		this(lines, new CaseInsensitiveMap<>());
	}
	...
	private static final Pattern PPDirectivePattern = Pattern.compile("##\\[([\\w:]+)\\]");
	private static final Matcher PPDirectiveMatcher = PPDirectivePattern.matcher("");
